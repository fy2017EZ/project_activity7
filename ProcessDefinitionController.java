...
//缺失流程部署ID属性版本，import org.activiti.api.process.model.ProcessDefinition;
    ...
    @GetMapping(value = "/getDefinitions")
    public AjaxResponse getDefinitions(@RequestParam(value = "userName")String userName) {

        try {
            Integer flag = 0;
            UserInfoBean userInfoBean = userInfoBeanMapper.selectByUsername(userName);
            String[] split = userInfoBean.getRoles().split(",");
    ...
    public AjaxResponse getDeployments(@RequestParam(value = "userName")String userName) {
        try {
            Integer flag = 0;
            UserInfoBean userInfoBean = userInfoBeanMapper.selectByUsername(userName);
            String[] split = userInfoBean.getRoles().split(",");
